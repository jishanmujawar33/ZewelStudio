...
export default function HomePage() {
  /* ─── Build featured products from real collection data ─── */
  const featuredProducts = useMemo(() => {
    const picks: { name: string; image: string; category: string; slug: string }[] = [];
    for (const [slug, indices] of Object.entries(FEATURED_PICK_INDICES)) {
      const cat = CATEGORIES.find((c) => c.slug === slug);
      if (!cat) continue;
      for (const idx of indices) {
        const img = cat.images[idx];
        if (!img) continue;
        picks.push({ name: img.alt, image: img.url, category: cat.name, slug: cat.slug });
      }
    }
    return picks;
  }, []);
